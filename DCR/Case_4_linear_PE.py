...
class NFTOMO(nn.Module):
    def __init__(self):
         ...
         self.b1 = nn.Sequential(nn.Linear(32,128), nn.LeakyReLU(inplace = True, negative_slope=0.2))
...
def input_mapping(x, B, L = 1):
  if B is None:
    return x
  else:
    cof_ = L/2
    x_proj = (cof_*torch.pi*x) @ B.T
  return torch.cat([torch.sin(x_proj), torch.cos(x_proj)], axis=-1)

num_L = 8
x = input_mapping(input_.detach(), torch.eye(2, dtype=torch.float32), L = 1)
for i in range(num_L- 1):
  x_ = input_mapping(input_.detach(), torch.eye(2, dtype=torch.float32), L = i+2)
  x = torch.concatenate([x, x_], axis=-1)
...
# files to work with
dir_path = "./Case_4_truncated_z_225_hete_background"
